...
# detect-upstream.sh
#
# Queries meganz/MEGAsync for the latest _Win release tag and decides
...
if [[ -z "$latest_tag" ]]; then
  echo "ERROR: no v*_Win tag found on ${UPSTREAM_REPO}" >&2
  exit 1
fi

# v6.3.0.1_Win -> 6.3.0.1
upstream_version="${latest_tag#v}"
upstream_version="${upstream_version%${TAG_SUFFIX}}"

echo "latest upstream Win tag: $latest_tag (version $upstream_version)"

# --- check if we already have a release for this version --------------------
release_tag="win-v${upstream_version}"
